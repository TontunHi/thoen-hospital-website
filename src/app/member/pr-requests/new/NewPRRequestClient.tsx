...
export default function NewPRRequestClient({ requester }: { requester: RequesterInfo }) {
  ...
  const [urgency, setUrgency] = useState<string>('ไม่ด่วน')
  ...
  const [jobTypeOther, setJobTypeOther] = useState<string>('')
  ...
  const [hospitalArea, setHospitalArea] = useState<string>('')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok && data.success) {
        router.push('/member/pr-requests')
      } else {
  ...
  return (
    <div className="prRequestsContainer">
      <Link href="/member/pr-requests" className="backLink">
        <ArrowLeft size={16} />
      ...
      <div className="pageHeader">
        <div>
          <h1>สร้างเอกสารคำขอส่งผลิตสื่อประชาสัมพันธ์</h1>
          <p className="pageSubtitle">กรอกข้อมูลรายละเอียดของสื่อที่ต้องการสั่งผลิต พร้อมกำหนดคิวขอลายเซ็นอนุมัติออนไลน์</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="formCard">
        ...
        <div className="formActions">
          <Link href="/member/pr-requests" className="submitBtn" style={{ background: '#64748b', boxShadow: 'none' }}>
            ยกเลิก
          </Link>
