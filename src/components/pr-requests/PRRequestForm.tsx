...
export interface PRRequestInitialData {
  id: number
  title: string
  urgency: string
  orderDate: string
...
export interface PRRequestFormProps {
  ...
  initialData?: PRRequestInitialData
}

export default function PRRequestForm({ mode, requester, initialData }: PRRequestFormProps) {
  const router = useRouter()
  const isPrOfficer = requester.position && requester.position.includes('นักประชาสัมพันธ์')
  const backUrl = mode === 'edit' && isPrOfficer ? '/member/approvals' : '/member/pr-requests'

  const [hasCost, setHasCost] = useState<boolean>(initialData ? initialData.has_cost === 1 : false)
  const [title, setTitle] = useState<string>(initialData?.title || '')
  const [urgency, setUrgency] = useState<string>(initialData?.urgency || 'ไม่ด่วน')
  const [orderDate, setOrderDate] = useState<string>(
    initialData?.orderDate || new Date().toISOString().split('T')[0]
  )
  const [targetDate, setTargetDate] = useState<string>(initialData?.targetDate || '')

  // Job Types checkboxes
  const initialJobTypes = initialData?.jobType || []
  ...
  const [jobTypeOther, setJobTypeOther] = useState<string>(initialData?.jobTypeOther || '')
  ...
  // Channels checkboxes
  const initialChannels = initialData?.channels || []
  const hospitalItem = initialChannels.find((c) => c.startsWith('ในอาคารโรงพยาบาลบริเวณ'))
  const initialHospitalArea = hospitalItem
    ? hospitalItem.replace('ในอาคารโรงพยาบาลบริเวณ', '').trim()
    : ''
  ...
  return (
    <div className="prRequestsContainer">
      <Link href={backUrl} className="backLink">
        <ArrowLeft size={16} />
        {mode === 'edit' ? 'ย้อนกลับ' : 'ย้อนกลับรายการทั้งหมด'}
      </Link>
      ...
      <div className="pageHeader">
        <div>
          <h1>
            {mode === 'edit'
              ? `แก้ไขเอกสารคำขอส่งผลิตสื่อ (ID: #${initialData?.id})`
              : 'สร้างเอกสารคำขอส่งผลิตสื่อประชาสัมพันธ์'}
          </h1>
          <p className="pageSubtitle">
            {mode === 'edit'
              ? 'ปรับปรุงรายละเอียดของสื่อที่ต้องการสั่งผลิต (แก้ไขได้เฉพาะสถานะรอดำเนินการ)'
              : 'กรอกข้อมูลรายละเอียดของสื่อที่ต้องการสั่งผลิต พร้อมกำหนดคิวขอลายเซ็นอนุมัติออนไลน์'}
          </p>
        </div>
      </div>
        ...
        <div className="formGrid">
          ...
          <div className="formGroup" style={{ marginTop: '8px' }}>
            ...
            {attachments.length > 0 && (
              ...
              >
                <span style={{ fontSize: '13px', fontWeight: 700, color: '#475569', marginBottom: '4px' }}>
                  ไฟล์แนบทั้งหมด ({attachments.length} ไฟล์):
                </span>
        ...
        <div className="formActions">
          ...
          <button type="submit" className="submitBtn" disabled={saving}>
            {saving ? (
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Check size={16} />
                {mode === 'edit' ? 'บันทึกการแก้ไข' : 'ส่งเอกสารคำขอ'}
              </span>
            )}
          </button>
        </div>
